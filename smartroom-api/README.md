...
## Pair new Devices
Pairing devices to the zigbee network and adding them to the API is NOT connected. Meaning, you have to manually add the device to zigbee and the add the device to the API with the id used in the zigbee network. 

To add a device use the respective endpoint based on the device type (plug, sensor, light). By adding the device, the subscriber will start to listen for messages coming from this device and the database will be able to store and query timeseries status about the device. 
